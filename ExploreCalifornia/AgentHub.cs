...
namespace ExploreCalifornia
{
    ...
    public class AgentHub : Hub
    {
        ...
        public async Task SendAgentMessage(Guid roomId, string text)
        {
            ...
            //When you need to interact with a hub from other classes like controller services or other hubs in this case,
            //You can inject iHubContext at the top
            await _chatHub.Clients.Group(roomId.ToString()).SendAsync("RecieveMessage", message.SenderName, message.SentAt, message.Text);
        }

        public async Task LoadHistory(Guid roomId)
        {
            var history = await _chatRoomService.GetMessageHistory(roomId);
            await Clients.Caller.SendAsync("RecieveMessages", history);
        }
    }
}
